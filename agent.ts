// Minimal trusted agent: authenticates, then runs one shielded read.
// Keeps scope tiny for 24h submission: auth + one verifiable action + logs.
...
if (!privateKey) {
  console.error("Missing T3N_DEMO_KEY");
  process.exit(1);
}
...
console.log("agent authenticated:", did);
// One shielded action: extend here with veil/swap call once key is live.
console.log("agent ready - add veil transfer here");
